<template>
    <div id="alert-categories">
        ...
        <div v-else>
            <v-select v-model="selectedCategory" label="Select to filter grid items" :items="categories" @update:modelValue="(selection) => selectionChanged(selection)"></v-select>
        </div>
    </div>
</template>
